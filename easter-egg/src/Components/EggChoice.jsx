...
class EggChoice extends React.Component {
  ...
  handleSubmit(event) {
    ...
    axios.get("http://easteregg.wildcodeschool.fr/api/eggs").then(resultat => {
  ...
  render() {
    ...
    const filteredEggs = this.state.eggsList.filter(egg => {
      return (
        egg.caliber === this.state.filter1 && egg.rarity === this.state.filter2
      );
    });
        ...
        <form onSubmit={this.handleSubmit}>
          <label className="text-color">
            ...
            <select
              ...
              onChange={this.handleChangeFilter1}
            >
              {caliber.map((calib, i) => {
                return (
                  <option value={calib} key={i}>
                    {caliber[i]}
                  </option>
                );
              })}
